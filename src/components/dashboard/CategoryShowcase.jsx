import { ChevronDownIcon, ChevronRightIcon, PresentationChartBarIcon, TrashIcon } from "@heroicons/react/24/solid";
import { Accordion, AccordionBody, AccordionHeader, List, ListItem, ListItemPrefix, ListItemSuffix, Typography } from "@material-tailwind/react";
...
const deleteCateroy =(categoryId)=>{
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            axios
            .delete(`https://nppinternational-server.vercel.app/deleteCategory/${categoryId}`,)
...
console.log(categoryId, subcategorySlug);
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            axios
            .delete(`https://nppinternational-server.vercel.app/deleteCategory/${categoryId}/${subcategorySlug}`,)
            .then(function (response) {
              console.log(response);
              if (response.status == 200) {
                Swal.fire(
                    ...
                    <div key={index}>
                        ...
                        >
                          ...
                          <ListItem className='p-0' selected={openAccordions[index]}>
                            ...
                            <ListItemSuffix>
                                        <TrashIcon onClick={()=> deleteCateroy(singleData._id)} className='h-5 text-red-500 w-5' />
                                    </ListItemSuffix>
                          </div>
                                ...
                                <List className='p-0'>
                                    {singleData.subCategories?.map((subcategory, subIndex) => (
                                        <ListItem key={subIndex}>
                                            ...
                                            <ListItemSuffix>
                                        <TrashIcon onClick={()=>deleteSubCateroy(singleData._id,subcategory.subcategorySlug)}  className='h-5 text-red-500 w-5' />
                                    </ListItemSuffix>
                                        </ListItem>
                                    ))}
                                </List>
